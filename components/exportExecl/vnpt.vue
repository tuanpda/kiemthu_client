  ...
  methods: {
    ...
    async exportExcel() {
      ...
      this.data_execl.forEach((item, index) => {
        ...
        row.getCell(6).value = `${item.maphuongan} - ${item.tenphuongan}`;
        row.getCell(7).value = "20";

        if (item.ngaybienlai) {
          const [datePart] = item.ngaybienlai.split(" ");
          const [day, month, year] = datePart.split("-");
          const formattedDate = `${day}/${month}/${year}`;
          row.getCell(8).value = formattedDate;
        }

        row.getCell(9).value = item.sobienlai;

        if (item.manguoithu == null || item.manguoithu == "") {
          row.getCell(10).value = "Mặc định";
          row.getCell(11).value = item.tienluongcs;
        } else {
          if (item.manguoithu !== 1) {
            row.getCell(10).value = item.manguoithu;
            if (item.manguoithu == 2) row.getCell(11).value = "1638000";
            else if (item.manguoithu == 3) row.getCell(11).value = "1404000";
            else if (item.manguoithu == 4) row.getCell(11).value = "1170000";
            else if (item.manguoithu == 5) row.getCell(11).value = "936000";
          } else {
            row.getCell(10).value = "Mặc định";
            row.getCell(11).value = item.tienluongcs;
          }
        ...
        row.getCell(12).value = Number(item.sotien);
        row.getCell(14).value = item.tungay;
        row.getCell(17).value = item.tentinh;
        row.getCell(18).value = item.matinh;
        row.getCell(19).value = item.tenquanhuyen;
        row.getCell(20).value = item.maquanhuyen;
        row.getCell(21).value = item.tenxaphuong;
        row.getCell(22).value = item.maxaphuong;
        row.getCell(23).value = item.tothon;
        row.getCell(24).value = item.maphuongthucdong;
        row.getCell(25).value = `Số biên lai: ${item.sobienlai}. Người nhập: ${item.tennguoitao}`;
        row.getCell(26).value = item.ngaysinh;
        row.getCell(27).value = item.gioitinh === "Nam" ? "1" : "0";
        row.getCell(28).value = item.cccd;

        const maNhanVienThu = item.sohoso.split("/").pop();
        row.getCell(29).value = `NV${maNhanVienThu}`;
        row.getCell(30).value = item.tentinh;
        row.getCell(31).value = item.matinh;
        row.getCell(32).value = item.tenbenhvien;
        row.getCell(33).value = item.mabenhvien.slice(2);

        // cột TK1
        row.getCell(35).value = "X";
        row.getCell(35).fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FFFF00" },
        ...
        row.getCell(36).value = item.cccd;
        row.getCell(54).value = item.tennguoitao;
        row.getCell(56).value = item.dienthoai;
        row.getCell(72).value = item.ghichu;
