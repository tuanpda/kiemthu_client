                  ...
                  :get-option-label="
                    (val) => {
                      if (typeof val === 'string') {
                        const found = item.info_xaphuong.find(
                          (x) => x.ward_code === val
                        );
...
import lineImage from "~/assets/images/line-hori.png";
  ...
  methods: {
    ...
    async findNguoihuong(masobhxh, index) {
      ...
      try {
        ...
        this.isLoading = true;
        const res = await this.$axios.post(`/api/kekhai/getinfo-bhxh`, dataFind);
          ...
          if (result.isConfirmed) {
            ...
            try {
              for (let i = 0; i < this.items.length; i++) {
                ...
                // console.log('Đến tháng mới: ', this.items[i].denthang);
    ...
    tinhTienPhaiDong(madoituong, muctiendong, maphuongthucdong, tuthang, index) {     
      ...
      const tienDiaPhuongHoTro =
        castSubTwhotro * (this.tylediaphuonghotroIl / 100);
      console.log(
      ...
      if (thangTruoc01072025 > 0) {
        ...
        console.log(
          "TỔNG MỨC HỖ TRỢ CHỈ CÓ NSNN MỨC MỚI CHO TOÀN BỘ QUÁ TRÌNH ĐÓNG"
        );
        // Tiền trung ương hỗ trợ (danh mục hỗ trợ cũ)
        const tienTrungUongHoTro = castSubTwhotro * (tyleHotroTW / 100);
        const tongTienHoTro = tienTrungUongHoTro + tienDiaPhuongHoTro;
    ...
    async intestBienLai() {
      const data = {
        ...
        createdBy: "",
        sobienlai: "",
        ngaybienlai: "25-06-2025 10:04:13",
        maloaihinh: "",
        tothon: "",
        tenquanhuyen: "",
        tentinh: "",
    ...
    async inBienLaiDientu(data) {
      ...
      doc.text(`Ngày ký: ${ngayBienLai}`, toadoXInfo + 124, toadoYInfo + 58, {
        fontWeight: "bold",
      });
